...
//    UIImageView *_yeellow = [[UIImageView alloc] initWithFrame:CGRectMake(_scrollView.frame.size.width/2-36, 4, 72, 117-19)];
//    _yeellow.image = [UIImage imageNamed:@"当前黄色"];
//    [superViewOfScrollView addSubview:_yeellow];
    ...
    switch (orientation) {
        case PagedFlowViewOrientationHorizontal:{
            ...
            for (NSInteger i = _visibleRange.location; i < _visibleRange.location + _visibleRange.length; i++) {
                ...
                if (delta < _pageSize.width) {//缩小 凸起左边
                    ...
                    for (id viewed in cell.subviews) {
                        
                        if ([[viewed class] isEqual:[UIImageView class]]) {
                            
                            UIImageView *veiede = (UIImageView *)viewed;
                            
                            veiede.frame = CGRectMake(0, 0, cell.frame.size.width, cell.frame.size.height-20);
                        }
                        
                        if ([[viewed class] isEqual:[UILabel class]]) {
                            
                            UILabel *veiedee = (UILabel *)viewed;
                            
                            veiedee.frame = CGRectMake(0, cell.frame.size.height-21, cell.frame.size.width, 20);
                        }
...
//                for (UIView *vieweddd in cell.subviews) {
//                    [vieweddd removeFromSuperview];
//                }
//                UILabel *baneed = [[UILabel alloc] initWithFrame:CGRectMake(0, cell.frame.size.height-30, cell.frame.size.width, 16)];
//                baneed.text = [NSString stringWithFormat:@"第%ld个",(long)pageIndex];
//                baneed.textAlignment = 1;
//                baneed.textColor = [UIColor whiteColor];
//                [baneed setFont:[UIFont systemFontOfSize:12]];
//                baneed.backgroundColor = [UIColor clearColor];
//                
//                UIImageView *Imageed = [[UIImageView alloc] init];
//                Imageed.frame = CGRectMake(0, 0, cell.frame.size.width, cell.frame.size.height-20);
//                Imageed.image = [UIImage imageNamed:@"商品图标"];
//                [cell addSubview:Imageed];
//                
//                [cell addSubview:baneed];
                break;
            case PagedFlowViewOrientationVertical:
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    switch (orientation) {
        case PagedFlowViewOrientationHorizontal:
            pageIndex = floor((_scrollView.contentOffset.x+1) / _pageSize.width);
            break;
        case PagedFlowViewOrientationVertical:
